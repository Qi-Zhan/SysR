...
impl Display for RError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            RError::InvalidInstruction(s) => {
                write!(f, "{}: {}", ("InvalidInstruction").bold().red(), s)
            }
            RError::InvalidCode(code) => write!(f, "{}: {:b}", ("InvalidCode").bold().red(), code),
            RError::InvalidRegister(reg) => {
                write!(f, "{}: {}", ("InvalidRegister").bold().red(), reg)
            }
            RError::InvalidMem(s) => write!(f, "{}: {:#x}", ("InvalidMem").bold().red(), s),
            RError::AddressMisaligned(s) => {
                write!(f, "{}: {:#x}", ("AddressMisaligned").bold().red(), s)
            }
            RError::InvalidAssembly(s) => write!(f, "{}: {}", ("InvalidAssembly").bold().red(), s),
            RError::Ebreak(return_code) => match *return_code {
                0 => write!(f, "{}", ("program exited with code 0").bold().green()),
                _ => write!(
                    f,
                    "{} with code {}",
                    ("program exited").bold().red(),
                    return_code
                ),
            },
