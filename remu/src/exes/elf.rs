use super::{
    elformat::{EI_MAG1, EI_MAG2, EI_MAG3, EI_MAGO},
    Exe,
};
    ...
    ($name:ident) => {
        impl $name {
            pub fn show_header(&self) {
                ...
                println!(
                    "  Class:                             {}",
                    EiClass::from_u8(self.header.ident[4]).unwrap()
                );
                println!(
                    "  Data:                              {}",
                    EiData::from_u8(self.header.ident[5]).unwrap()
                );
                println!(
                    "  Version:                           {}",
                    EiVersion::from_u8(self.header.ident[6]).unwrap()
                );
                println!(
                    "  OS/ABI:                            {}",
                    EiOsAbi::from_u8(self.header.ident[7]).unwrap()
                );
                println!(
                    "  ABI Version:                       {}",
                    self.header.ident[8]
                ...
                println!(
                    "  Type:                              {}",
                    EType::from_u16(self.header.e_type).unwrap()
                );
                println!(
                    "  Machine:                           {}",
                    EMachine::from_u16(self.header.machine).unwrap()
                );
                println!(
                    "  Version:                           {}",
                    self.header.version
        ...
        impl $name {
            pub fn show_program_headers(&self) {
                println!(
                    ...
                    self.header.phnum, self.header.phoff
                );
                ...
                for header in self.program_headers.iter() {
                    ...
                    println!(
                        "0x{:06x} 0x{:08x} 0x{:08x} 0x{:05x} 0x{:05x} {:>20} 0x{:06x}",
                        header.offset,
                        header.vaddr,
                        header.paddr,
                        header.filesz,
        ...
        fn parse(input: &[u8]) -> Result<Self, RError> {
            ...
            if header.phoff != 0 {
                ...
                for _ in 0..header.phnum {
                    ...
                    let phdr = ProgramHeader::linearparse(
                        &input[index..(index + header.phentsize as usize)],
                    );
            ...
            if header.shoff != 0 {
                ...
                for _ in 0..header.shnum {
                    ...
                    let shdr = SectionHeader::linearparse(
                        &input[index..index + header.shentsize as usize],
                    );
